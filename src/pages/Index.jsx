...
const Index = () => {
  ...
  const [states, setStates] = useState(fields.allState || []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      // if something is wrong with stored JSON, fall back to all states
      setStates(fields.allState || []);
      return;
    }
    ...
    const rawAccess =
      user.accessState || user.allowedStates || user.stateAccess || [];

    let accessStates = [];

    if (Array.isArray(rawAccess)) {
      accessStates = rawAccess;
    ...
    if (!accessStates || accessStates.length === 0) {
      // if no restriction, allow all states
      setStates(fields.allState || []);
      return;
    }

    // Filter by name or code, so it works whatever is stored
    const allowedStates =
      (fields.allState || []).filter((s) => {
        return (
          accessStates.includes(s.name) ||
          accessStates.includes(s.code) ||
          accessStates.includes(String(s.name).toUpperCase()) ||
          accessStates.includes(String(s.code).toUpperCase())
        );
      }) || [];
    ...
    if (allowedStates.length > 0) {
      ...
      // fallback if mapping didn't match
      setStates(fields.allState || []);
    }
  }, []);
    ...
    <>
      ...
      <div className="box box--main">
        ...
        <ul className="box__steps mt-2">
          ...
          <li className="box__step">
            Enter <span>'vehicle No.'</span> and click{" "}
            <span>'Get Details'</span> button to fill the details.
          </li>
          <li className="box__step">
